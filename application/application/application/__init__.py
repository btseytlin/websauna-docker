...
class Initializer(websauna.system.Initializer):
    """An initialization configuration used for starting application.
    ...
    def configure_static(self):
        ...
        self.config.registry.static_asset_policy.add_static_view('application-static', 'application.application:static')

    def configure_templates(self):
        """Include our package templates folder in Jinja 2 configuration."""
        super(Initializer, self).configure_templates()

        self.config.add_jinja2_search_path('application.application:templates', name='.html', prepend=True)  # HTML templates for pages
        self.config.add_jinja2_search_path('application.application:templates', name='.txt', prepend=True)  # Plain text email templates (if any)
        self.config.add_jinja2_search_path('application.application:templates', name='.xml', prepend=True)  # Sitemap and misc XML files (if any)
